...
(async () => {
  try {
    ...
    try {
      const [cols] = await rootConnection.query(
        "SHOW COLUMNS FROM users LIKE 'name'",
      );
      if (cols.length === 0) {
        await rootConnection.query(
          "ALTER TABLE users ADD COLUMN name VARCHAR(100) NOT NULL DEFAULT 'Thành viên'",
        );
      }
    } catch {
    ...
    try {
      const [trackCols] = await rootConnection.query(
        "SHOW COLUMNS FROM mentors LIKE 'track'",
      );
      if (trackCols.length === 0) {
        await rootConnection.query(
          "ALTER TABLE mentors ADD COLUMN track VARCHAR(100) NOT NULL DEFAULT 'Lập trình ứng dụng'",
        );
      }
    } catch {
    ...
    if (error.code === "ER_ACCESS_DENIED_ERROR" || error.errno === 1045) {
      console.error(
        "🔑 Sai mật khẩu root MySQL! Vui lòng kiểm tra lại DB_PASSWORD trong file server/.env",
      );
    } else {
      console.error(
        "💡 Vui lòng kiểm tra lại dịch vụ MySQL Server trên localhost:3306.",
      );
    }
  }
})();
